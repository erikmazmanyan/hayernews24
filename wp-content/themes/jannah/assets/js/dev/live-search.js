...
/*
 * MODIFIED BY TIELABS
 * Fixed RTL position issues
 * Add condition for the Full Screen Search
	...
	function Autocomplete(el, options) {
		var noop = function () { },
			...
			defaults = {
				...
				onSearchError: noop,
				preserveInput: false,
				containerClass: '#autocomplete-suggestions', // TieLabs use ID
				tabDisabled: false,
				dataType: 'text',
				currentRequest: null,
	...
	Autocomplete.prototype = {
		...
		initialize: function () {
			...
			that.killerFn = function (e) {
				if ($(e.target).closest( /*'.' + */ that.options.containerClass).length === 0) { // By TieLabs
			...
			// TieLabs Prevent the js from creating the Search results DIV
			// that.suggestionsContainer = Autocomplete.utils.createNode(options.containerClass);

			that.suggestionsContainer = $(options.containerClass);
		...
		//By TieLabs
			...
			if( that.el.closest('.pop-up-live-search').length > 0 ){
			  ...
			  styles.maxWidth = w+'px'; //(w - 30)+'px';
				...
				if( is_RTL ){
					var leftPosition = Math.floor(offset.left+that.el.outerWidth()-that.options.width+1);
				}
				else{
					var leftPosition = Math.floor(offset.left-1);
				}

			  var styles = { 'top': offset.top, 'left': leftPosition };
			}
			else{
